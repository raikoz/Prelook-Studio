...
const IMAGE_MODEL_NAME = 'gemini-2.0-flash-exp'; // Supports early access image generation features
const CHAT_MODEL_NAME = 'gemini-1.5-flash';

// Helper to generate a single image view via API
...
export const chatWithAI = async (
  ...
  switch (role) {
    case 'PARTNER':
      systemInstruction = "You are a friendly support agent for Prelook Studio Salon Partners. Help them manage bookings, optimize revenue, use the dashboard, and resolve technical issues with their salon listings. Keep answers concise and business-oriented.";
      break;
    case 'REGION':
      systemInstruction = "You are an expert business analyst for Prelook Franchise Owners. Help them interpret regional data, performance metrics across multiple salons, analyze revenue trends, and suggest growth strategies. Be professional, data-driven, and insightful.";
      break;
    default:
      systemInstruction = "You are a helpful and stylish virtual assistant for Prelook Studio customers. Assist them with booking appointments, finding salons, app navigation, and advice on hairstyles. Be fun, polite, and encouraging.";
      break;
  }

  try {
    // Map history to format expected by API/SDK logic
