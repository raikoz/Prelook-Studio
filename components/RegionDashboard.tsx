...
export const RegionDashboard: React.FC<RegionDashboardProps> = ({ user, onLogout, onTopUp }) => {
    // State for Salons under RM
    const [salons, setSalons] = React.useState([
        { id: '1', name: 'Luxe Studio - Saheed Nagar', revenue: '₹4.2M', growth: '+15%', status: 'RUNNING' },
        { id: '2', name: 'Prelook X - Esplanade', revenue: '₹3.8M', growth: '+8%', status: 'RUNNING' },
        { id: '3', name: 'Urban Cuts - Patia', revenue: '₹2.1M', growth: '-2%', status: 'PAUSED' },
        { id: '4', name: 'Style Hub - Cuttack', revenue: '₹1.8M', growth: '+5%', status: 'RUNNING' },
        { id: '5', name: 'Glamour Point - Puri', revenue: '₹1.2M', growth: '+10%', status: 'PAUSED' },
    ]);
    ...
    const activeCount = salons.filter(s => s.status === 'RUNNING').length;
    const monthlyCost = activeCount * 4999;

    // Derived Stats
    const stats = [
        { label: 'Total Revenue', value: '₹12.4M', change: '+12%', icon: DollarSign, color: 'text-green-600' },
        { label: 'Active Salons', value: activeCount.toString(), change: `${salons.length} Total`, icon: Map, color: 'text-brand-600' },
        { label: 'Network Cost', value: `₹${monthlyCost.toLocaleString()}`, change: '₹4999/salon', icon: Wallet, color: 'text-red-600' },
                ...
                <div className="max-w-7xl mx-auto space-y-8">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                        {stats.map((stat, idx) => (
                            <div key={idx} className="bg-white p-6 rounded-2xl border border-brand-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                                ...
                                <div>
                                    ...
                                    <span className="text-xs font-bold text-brand-500 bg-brand-50 px-2 py-0.5 rounded-full inline-block mt-2">
                                        {stat.change}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className="bg-brand-900 text-white p-6 rounded-2xl shadow-xl flex flex-col justify-between">
                            <div className="flex justify-between items-start mb-4">
                                <span className="text-xs font-bold uppercase text-brand-300 tracking-widest">Upcoming Invoice</span>
                                <Wallet className="w-5 h-5 text-accent" />
                            </div>
                            <div className="flex items-end justify-between">
                                <div>
                                    <span className="text-3xl font-serif font-medium">₹{monthlyCost.toLocaleString()}</span>
                                    <span className="text-[10px] text-brand-300 block">+ GST</span>
                                </div>
                                <button
                                    onClick={() => onTopUp({ type: 'FUND', name: 'Regional Network Payment', price: monthlyCost })}
                                    className="px-4 py-2 bg-white text-brand-900 rounded-lg text-xs font-bold font-serif hover:bg-brand-100 transition-colors"
                                >
                                    Pay Now
                                </button>
                            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 bg-white rounded-3xl border border-brand-100 shadow-sm overflow-hidden">
                            <div className="p-6 border-b border-brand-100 flex items-center justify-between">
                                <h3 className="font-serif text-xl text-brand-900">Salon Performance & Status</h3>
                                <button className="text-brand-500 hover:text-brand-900 text-xs font-bold uppercase tracking-wider">Manage All</button>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-left text-sm">
                                    ...
                                    <tbody className="divide-y divide-brand-100">
                                        {salons.map((salon) => (
                                            <tr key={salon.id} className="hover:bg-brand-50/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    ${salon.status === 'RUNNING' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}
                                                `}>
                                                        <div className={`w-2 h-2 rounded-full ${salon.status === 'RUNNING' ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                                                        {salon.status}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <button
                                                        ...
                                                        className={`text-xs font-bold underline ${salon.status === 'RUNNING' ? 'text-red-500 hover:text-red-700' : 'text-green-600 hover:text-green-800'}`}
                                                    >
                                                        {salon.status === 'RUNNING' ? 'Pause' : 'Activate'}
                                                    </button>
                                                </td>
                                            </tr>
                                        ))}
